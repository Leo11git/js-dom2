...
function func() {
	for (i = 0; i <= cats.length; i++) {
		...
		let pfav = document.createElement("p");
		...
		p.innerHTML = 'Возраст: ' + cats[i].age;
		name.innerHTML = 'Имя: ' + cats[i].name;
		prate.innerHTML = 'Рейтинг: ' + cats[i].rate;
		if (cats[i].favourite) {
			pfav.innerHTML = 'Любимый: Да';
		} else {
			pfav.innerHTML = 'Любимый: Нет';
		}
		desc.innerHTML = 'Описание: ' + cats[i].description;
		
		document.body.appendChild(div)
		let z = document.getElementById("IdDiv"+i)
